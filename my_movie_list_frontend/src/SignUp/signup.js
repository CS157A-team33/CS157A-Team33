...
class SignUp extends Component {
  ...
  signUp = _ => {
    fetch(
      `http://localhost:4040/signup?email=${this.state.user.email}&password=${this.state.user.password}&username=${this.state.user.name}`
            ...
            <MDBCard style={{ marginTop: "25%" }}>
              <MDBCardBody className="mx-4">
                ...
                <MDBInput
                  ...
                  success="right"
                  onChange={i =>
                    this.setState({ user: { ...user, name: i.target.value } })
                  }
                />
                <MDBInput
                  label="Email"
